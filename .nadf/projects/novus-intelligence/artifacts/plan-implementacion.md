...
> **Aprobación pendiente:** Este plan permanece en `draft` hasta Plan Review por **architect-agent** (paso 6). Ningún agente Executor debe modificar código productivo hasta `status: approved`.
...
| `status` | **draft** — pendiente aprobación humana vía architect-agent |
...
### Constraints de esta ejecución

| Constraint | Estado |
|------------|--------|
| `NO_PRODUCTIVE_CODE` | Cumplido — solo artefactos de planificación |
| `NO_LOVABLE_CODE_COPY` | Gate bloqueante en Execution |
| `NO_DEPLOY` | Infra se propone; despliegue bloqueado |
| `NO_SECRETS_IN_REPO` | Variables sensibles solo en AWS |
| `PLAN_STATUS_MUST_BE_DRAFT` | `status: draft` |
| `TARGET_DEV_REGION_SA_EAST_1` | Región DEV: sa-east-1 |
...
|--------|-----------|----------------|
| **R-001** Modo demo contacto | Alta | Eliminar fallback demo; API obligatoria; `VITE_DEMO_MODE=false` en prod; QA valida error sin API |
...
**Mitigación R-001 (modo demo):** El frontend productivo **no debe** replicar la lógica de `src/lib/api/contact.ts` de Lovable que simula éxito con `requestId: demo-{timestamp}`. Si la API no responde, el usuario debe ver un error explícito.
...
|---|-------|--------|-------------------------|
| 7.1 | Verificar alineación `environments/dev.yml` con sa-east-1 | cloud-agent | Región, URLs y stack documentados |
...
| 9.9 | Paridad visual exacta | diff ≤ 0.2% en rutas /, /about, /services, /contact (ADR-0006) |
...
| Paso | Agente | Acción |
|------|--------|--------|
| 2 (actual) | **planner-agent** | Generar plan + tareas-ejecutor.json (`draft`) |
