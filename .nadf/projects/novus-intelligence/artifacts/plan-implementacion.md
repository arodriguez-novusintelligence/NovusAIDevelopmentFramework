...
> **Nota de alineación regional:** `environments/dev.yml` declara `region: sa-east-1` (reconciliado). El target operativo DEV es **sa-east-1**. QA/prod permanecen en `us-east-1` (fuera de alcance DEV). Certificado ACM CloudFront sigue en `us-east-1` por requisito AWS.
...
|---|-------|--------|-------------------------|
| 7.1 | Verificar `environments/dev.yml` → región `sa-east-1` | cloud-agent | Archivo alineado con target DEV (TASK-INFRA-001 completado) |
...
| **cloud-agent** | Fase 7 (preparación IaC + deploy DEV) | Región DEV ya en `sa-east-1`; deploy DEV auto tras gates (ADR-0006) |
| **devops-agent** | Fase 7 (CI/pipeline) | `VITE_DEMO_MODE=false`; deploy DEV auto tras gates |

**Tareas previas de infra (no bloquean inicio de código):**

1. ~~Reconciliar región DEV `us-east-1` → `sa-east-1` en `environments/dev.yml`~~ — **Completado** (TASK-INFRA-001).
2. Normalizar nombres de variables email a `CONTACT_EMAIL_FROM` / `CONTACT_EMAIL_TO` (coherencia con especificación backend).
3. Despliegue DEV: automático tras gates NADF incl. `visual_exact_parity` (ADR-0006); PROD/QA requieren aprobación humana.
...
| 2026-07-14 | Revalidación paso-03: región DEV confirmada sa-east-1; plan mantiene `approved` | architect-agent |
