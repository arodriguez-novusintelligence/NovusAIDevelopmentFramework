...
> **Nota de gobernanza:** Este plan permanece en `draft` hasta aprobación humana y revisión por **architect-agent** (paso 6 — Plan Review). Ningún agente Executor debe modificar código productivo hasta que `status == approved`.
...
| Campo | Valor |
|-------|-------|
| `plan_id` | PLAN-NOVUS-LOVABLE-2026-07-16 |
| `intent_source` | cambios-lovable.json (lovable-analyzer-agent) |
| `status` | **draft** — pendiente Plan Review (architect-agent) |
...
- `NO_LOVABLE_CODE_COPY` — Traducción de intención únicamente.
...
- `TARGET_DEV_REGION_SA_EAST_1` — Toda propuesta cloud DEV apunta a región **sa-east-1** (confirmado en `environments/dev.yml` y `project-context.yml`).
...
|--------|-----------|----------------|
| **R-001** Modo demo contacto | Alta | Tareas explícitas Fase 6: eliminar fallback demo; API obligatoria; `VITE_DEMO_MODE=false` en prod; QA valida error sin API |
...
| 5.4 | Integración AWS SES (email notificación) | CHG-011 | Email enviado a destino configurado vía secret |
...
**Mitigación R-001 (modo demo):** El frontend productivo **no debe** replicar la lógica de `src/lib/api/contact.ts` de Lovable que retorna éxito simulado con `requestId: demo-{timestamp}`. Si la API no responde, el usuario debe ver un error explícito.
...
|---|-------|--------|-------------------------|
| 7.1 | Verificar alineación `environments/dev.yml` con región `sa-east-1` | cloud-agent | Configuración DEV coherente con project-context |
...
| — | architect-agent | Plan Review (paso 6) | Antes de Execution |
...
| Paso | Agente | Acción |
|------|--------|--------|
| 4 (actual) | **planner-agent** | `plan-implementacion.md` + `tareas-ejecutor.json` en status `draft` |
...
| Fecha | Acción | Agente |
|-------|--------|--------|
| 2026-07-16 | Plan regenerado en status `draft` (paso-02-generar-plan) | planner-agent |
