...
> **Aprobación pendiente:** Este plan requiere Plan Review por **architect-agent** (paso 6) antes de autorizar Execution. El planner-agent **no** puede aprobar el plan.
...
| Deploy con aprobación | `deploy_human_approval` | Ningún despliegue a QA/prod sin aprobación humana explícita; DEV auto-deploy solo tras gates (ADR-0006) | devops-agent / cloud-agent |
...
| Paridad visual exacta | `visual_exact_parity` | Diff visual ≤ 0.2% vs referencia Lovable en rutas DEV | visual-parity-agent |
...
- `NO_DEPLOY` — Infraestructura se propone; despliegue queda bloqueado hasta gates y aprobación según entorno.
- `NO_SECRETS_IN_REPO` — Solo nombres de variables documentados.
- `TARGET_DEV_REGION_SA_EAST_1` — Toda propuesta cloud DEV apunta a región **sa-east-1** (`environments/dev.yml`).
...
|--------|-----------|----------------|
| **R-001** Modo demo contacto | Alta | Tareas explícitas: eliminar fallback demo; API obligatoria; `VITE_DEMO_MODE=false` en prod; QA valida error sin API |
...
flowchart TD
    ...
    P3 --> P5[Fase 5: Backend contact API]
    P4 --> P6[Fase 6: Contacto frontend + integración]
    P5 --> P6
    P6 --> P7[Fase 7: Infra DEV sa-east-1]
    P7 --> P8[Fase 8: QA + Security + Review + Paridad visual]
```
...
4. **Infra DEV (Fase 7)** requiere artefactos de backend-impact-agent (`evaluacion-backend.md`, `especificacion-backend.md`); despliegue según gates ADR-0006.
...
| # | Tarea | Cambios | Criterios de aceptación |
|---|-------|---------|-------------------------|
| 5.1 | Implementar Lambda `novus-contact-handler` | CHG-011 | Handler Node.js 20; timeout 10s; región sa-east-1 |
...
**Contrato request/response:** según `backend-impact.md` y contrato OpenAPI en novus-nexus (referencia de intención, no código).
...
**API DEV esperada:** `https://api-dev.novusintelligence.com` (según `environments/dev.yml`, región sa-east-1).
...
**Bloqueo:** deploy según gates ADR-0006; QA/prod requieren aprobación humana

| # | Tarea | Agente | Criterios de aceptación |
|---|-------|--------|-------------------------|
| 7.1 | Validar alineación `environments/dev.yml` región sa-east-1 | cloud-agent | Config DEV coherente con project-context |
...
| 7.8 | **Despliegue DEV** | devops-agent + cloud-agent | Solo tras gates (plan_approved, qa_pass, security_pass, visual_exact_parity) |
...
**Agentes:** qa-agent, security-agent, reviewer-agent, visual-parity-agent  
...
| 9.9 | Paridad visual exacta | Diff ≤ 0.2% en `/`, `/about`, `/services`, `/contact` (viewports 1440×900, 768×1024, 390×844) |
...
|-------|--------|-----------|----------|
| — | backend-impact-agent | planning (paso 5) | Antes de Execution backend |
...
| 7 | cloud-agent + devops-agent | 7 | Tras especificación backend; deploy con gates |
| 8 | qa-agent + security-agent + reviewer-agent + visual-parity-agent | 9 | Tras ejecución |
...
| Paso | Agente | Acción |
|------|--------|--------|
| 4 (este paso) | **planner-agent** | Generar `plan-implementacion.md` + `tareas-ejecutor.json` en status **draft** |
| 5 | **backend-impact-agent** | `evaluacion-backend.md` + `especificacion-backend.md` |
| 6 | **architect-agent** | Plan Review → cambiar status a `approved` si cumple gates |
| 7+ | **frontend-integration-agent**, **backend-agent** | Execution (solo tras plan approved) |
| 7 (infra) | **cloud-agent**, **devops-agent** | Preparación IaC; deploy DEV tras gates ADR-0006 |
