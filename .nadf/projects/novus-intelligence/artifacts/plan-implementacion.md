...
> **Estado del plan:** `draft` — pendiente de Plan Review por **architect-agent** (paso 6). Ningún agente Executor debe modificar código productivo hasta `status: approved`.
...
| `NO_DEPLOY` | Infraestructura se propone; despliegue bloqueado |
| `NO_SECRETS_IN_REPO` | Solo nombres de variables documentados |
| `PLAN_STATUS_MUST_BE_DRAFT` | Aprobación humana vía architect-agent |
| `TARGET_DEV_REGION_SA_EAST_1` | DEV en región **sa-east-1** según `project-context.yml` y `environments/dev.yml` |
...
**Dependencias:** Fases 5 y 6 especificadas; `evaluacion-backend.md` y `especificacion-backend.md`  
**Bloqueo:** `deploy_human_approval` — solo preparar artefactos

| # | Tarea | Agente | Criterios de aceptación |
|---|-------|--------|-------------------------|
| 7.1 | Verificar alineación `environments/dev.yml` con región `sa-east-1` | cloud-agent | Config DEV coherente con `project-context.yml` |
...
| 9.9 | Paridad visual exacta | visual-parity-agent; diff ≤ 0.2% en rutas `/`, `/about`, `/services`, `/contact` |
...
| Fecha | Acción | Agente |
|-------|--------|--------|
| 2026-07-15 | Plan generado en status `draft` | planner-agent |
